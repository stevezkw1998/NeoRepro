...
We pinned and reproduced five public predictors, harmonized a presentation-prefiltered patient-matched pMHC multimer screen and an independent personalized-vaccine ELISPOT cohort, audited known training overlap, and evaluated pairwise common-support predictions. We report pooled discrimination, patient-level Top-K retrieval with patient bootstrap, sensitivity analyses, and transparent held-out baselines.

### Results

All three pinned predictors ran to completion. The initial 520-row TESLA fixture was entirely training-overlapped and was not used for performance claims. After excluding 45 exact PRIME2 overlaps from 17,520 IMPROVE records, 17,475 records from 70 patients remained. On the common set, PRIME achieved AUROC 0.597 and mean pMHC-pair Recall@20 0.260 among 60 positive-bearing patients, versus 0.546 and 0.146 for BigMHC. Transparent peptide baselines outperformed HLA-only baselines under both patient- and study-held-out fitting, while adding HLA to peptide features did not consistently improve over peptide features alone. A frozen independent extension evaluated five models on 2,315 overlap-filtered vaccine peptides with a distinct post-vaccination ELISPOT endpoint.
...
The core protocol was frozen in `RESEARCH_SPEC.md`; the external extension and primary NDCG@5 endpoint were separately frozen in `research/extension_protocol.json` before extension inference. Each predictor was assigned a pinned source revision, isolated environment, legal-access record, standardized adapter and evidence directory. Failed attempts were retained. MHCflurry 2.2.1 was run as a presentation predictor; BigMHC v1.0, PRIME 2.0, DeepImmuno-CNN and DeepHLApan were run as immunogenicity predictors. Scores were oriented so that larger values indicate stronger predicted evidence. No missing prediction was imputed.
...
|---|---:|---:|---:|---:|---:|
| BigMHC | 2,315 | 0.550 | 0.148 | 0.658 (0.606–0.707) | 0.816 |
| DeepHLApan | 2,315 | 0.488 | 0.131 | 0.580 (0.524–0.635) | 0.726 |
| DeepImmuno-CNN | 1,015 | 0.526 | 0.158 | 0.755 (0.691–0.816) | 0.925 |
| PRIME | 2,310 | 0.531 | 0.148 | 0.604 (0.546–0.660) | 0.770 |
...
The independent vaccine cohort strengthens that caution: on near-complete common support, BigMHC ranked administered vaccine peptides better than PRIME and DeepHLApan by the frozen patient NDCG@5 comparison, reversing the BigMHC–PRIME direction seen in IMPROVE. DeepImmuno-CNN covered less than half of the external cohort, and common-support intervals did not resolve its differences from the other immunogenicity models. This is substantive evidence of model-by-domain dependence, not evidence that one model is globally best. The extension is also biologically complementary rather than a pure replication because its post-vaccination ELISPOT endpoint and much smaller patient candidate sets differ from IMPROVE.
